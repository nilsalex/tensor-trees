...
void eval_fun (std::unique_ptr<Tree<Node>> const & t, std::set<std::map<size_t, mpq_class>> & eval_res_set) {
  ...
  std::cout << "Number of different combinations : " << 11 * 21 * 210 << std::endl;
      ...
      do {
        do {
          std::map<char, char> eval_map {
            ...
            {'p', metric_indices.first},
            {'q', metric_indices.second}
          };
          ...
          ++counter;
          int _progress = (100 * counter) / (11 * 21 * 210);
